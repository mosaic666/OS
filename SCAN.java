...
public class SCAN{
    ...
    public void way(){
    	...
    	if (direction==0) {
    		...
    		sum+=Math.abs(sresult[0]-bresult[0])+Math.abs(work-sresult[sresult.length-1]);
    		System.out.println();
    		...
    		sum+=Math.abs(bresult[bresult.length-1]-sresult[sresult.length-1])+Math.abs(work-bresult[0]);
    		System.out.println();
